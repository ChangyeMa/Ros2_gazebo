...
def camPreview(previewName, camID):
    ...
    if cam.isOpened():  # try to get the first frame
        rval, frame = cam.read()
        # size the resolution of the video stream
        cam.set(3, 640)
        cam.set(4, 480)
    else:
        rval = False
...
#     t_world_to_camera = np.array([0, 0, 1950])  # Assume the camera is at the origin in the world frame
...
#             # define a intrinsic camera matrix for the webcam EMEET C960
#             K = np.array([[1200, 0, 980], 
#                           [0, 1200, 540], 
#                           [0, 0, 1]])
